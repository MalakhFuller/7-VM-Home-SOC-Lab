...
5. **Lateral Movement** — _planned_ — moving across the domain once you're in. A multi-part phase of its own: PsExec/SMB, Pass-the-Hash, WinRM, and RDP, each as its own dual-SIEM detection writeup. From there the series keeps working down the catalog of what an attacker with a foothold actually does — persistence, credential abuse, exfiltration — one technique at a time.
...
The instrument is built and verified, and the attacks are underway. Password spraying, Kerberoasting, and privilege escalation (above) are all run through it end to end — each ending in custom detection rules written in both SIEMs, from the contrast between a loud spray and a patient one to an ACL-abuse chain caught at every stage it leaves a trace. Lateral movement is next — a multi-part phase in its own right (PsExec/SMB, Pass-the-Hash, WinRM, RDP). From there the series keeps walking the post-foothold catalog one technique at a time — persistence, credential abuse, exfiltration — each taken end to end through both SIEMs, the same way as everything above.
